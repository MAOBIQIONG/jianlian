package com.fh.controller.shop;
...
public class ShopController extends BaseController {  
	...
	public String queryList(Page page) throws Exception {
		...
		List<PageData> shops = this.shopService.listPageByParam(page);
		Object	total = this.shopService.findCount(page).get("counts");//查询记录的总行数 
	...
	public ModelAndView goEdit() throws Exception {
		...
		try {
			pd = getPageData();  
			pd = this.shopService.queryById(pd);   
			mv.addObject("pd",pd);  
			mv.setViewName("shop/shop_edit"); 
		} catch (Exception e) { 
			e.printStackTrace();
	 ...
	 public String save(MultipartHttpServletRequest request)throws Exception{ 
	     ...
	     try {  
	    	 ...
	    	 if(ID==null||"".equals(ID)){//新增  
	    		 ...
	    		 //保存店铺信息
	    		 ob=this.shopService.save(pd);  
	    	 }else{//修改 
	    		 pd.put("ID",ID); 
	    		 ob=this.shopService.edit(pd);  
	    	 }  
		 } catch (Exception e) {
