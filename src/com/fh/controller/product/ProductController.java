...
public class ProductController extends BaseController {  
	...
	public String queryList(Page page) throws Exception {
		...
		List<PageData> shops = this.prodService.listPageByParam(page);
		Object	total = this.prodService.findCount(page).get("counts");//查询记录的总行数 
	...
	public ModelAndView goEdit() throws Exception {
		...
		try {
			pd = getPageData();  
			List<PageData> data = this.prodService.findById(pd);   
	 ...
	 public String save(MultipartHttpServletRequest request)throws Exception{ 
	     ...
	     try {  
	    	 ...
	    	 if(id==null||"".equals(id)){//新增  
	    		 ...
	    		 ob=this.prodService.save(pd);   
	    	 }else{//修改 
	    		 pd.put("ID",id); 
	    		 ob=this.prodService.edit(pd);  
	    	 }  
		 } catch (Exception e) {
	...
	public String delete() {
		...
		try {
			pd = getPageData(); 
			ob=this.prodService.updateStatus(pd);  
			if(Integer.parseInt(ob.toString())>=1){
				getObj.put("statusCode", 200);
			}else{
				getObj.put("statusCode", 400);
	...
	public ModelAndView toImg() throws Exception {
		...
		try {
			...
			List<PageData> imgs =prodService.findImages(pd);  
			mv.addObject("imgs", imgs); 
		} catch (Exception e) { 
			e.printStackTrace();
		}    
		mv.setViewName("product/pro_img_list");  
		mv.addObject("pd", pd);
		return mv;
	}
	...
	public ModelAndView todescImg() throws Exception {
		...
		try {
			pd = getPageData();  
			List<PageData> imgs =prodService.finddescImages(pd);  
			mv.addObject("imgs", imgs); 
		} catch (Exception e) { 
			e.printStackTrace();
		}    
		mv.setViewName("product/pro_descimg_list");  
		mv.addObject("pd", pd);
		return mv;
	}
	...
	public ModelAndView toAddimg() throws Exception {
		...
		mv.setViewName("product/pro_img_add"); 
		mv.addObject("pd", pd);
		return mv;
	}
	...
	public JSONObject uploadImgs(MultipartHttpServletRequest request)throws Exception{
		...
		pd.put("ID", get32UUID()); 
		Object ob=prodService.saveImg(pd);
