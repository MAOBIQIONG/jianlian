...
import com.fh.service.system.pyq.PyqtpService;
...
@Controller
@RequestMapping({ "/pyqtp" })
public class pyqtpController extends BaseController {  
	
	@Resource(name = "pyqtpService")
	private PyqtpService pyqtpService; 
	...
	public ModelAndView toLookImgs() throws Exception {
		...
		try {
			pd = getPageData();  
			PageData pro=PyqServices.findById(pd); 
			pd.put("RELATED_ID",pd.getString("ID"));
			List<PageData> xmtp =pyqtpService.queryByPid(pd);  
			mv.addObject("pro", pro); 
			mv.addObject("xmtp", xmtp); 
		} catch (Exception e) { 
			e.printStackTrace();
		}    
		mv.setViewName("system/pyq/pyq_img_list");  
		return mv;
	}
	...
	public ModelAndView toAdd() throws Exception {
		...
		try {
			pd = getPageData(); 
			mv.addObject("RELATED_ID", pd.getString("RELATED_ID")); 
		} catch (Exception e) { 
			e.printStackTrace();
		}  
	    mv.setViewName("system/pyq/pyq_img_add");  
		return mv;
	}
	...
	public JSONObject uploadImgs(MultipartHttpServletRequest request)throws Exception{
		...
		PageData data=pyqtpService.queryMaxOrderby(pd);
		...
		Object ob=pyqtpService.save(pd);
