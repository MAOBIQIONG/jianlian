package com.fh.controller.system.Govers;
...
public class GoversController extends BaseController { 
	String appKey = "vnroth0kvfo6o";//替换成您的appkey
	String appSecret = "fEPFfFSclDYo";//替换成匹配上面key的secret
	RongCloud rongCloud = RongCloud.getInstance(appKey, appSecret);
	
	@Resource(name = "goversService")
	private GoversService goversService;
	...
	public ModelAndView list(Page page) throws Exception {
		...
		mv.setViewName("system/Goverstment/Govers_list");
		return mv;
	}
	...
	public String queryList(Page page) throws Exception {
		...
		List<PageData> projectsList = this.goversService.listByParam(page);
		total = this.goversService.findCount(page).get("counts");//查询记录的总行数
		getObj.put("sEcho", initEcho);// 不知道这个值有什么用
		getObj.put("iTotalRecords", total);//实际的行数
		getObj.put("iTotalDisplayRecords", total);//显示的行数,这个要和上面写的一样
		
		for(PageData pro:projectsList){ 
			ResultUtil.resetRes(pro,new String[]{"DUE_DATE","BID_DATE","AUDIT_DATE","MODIFY_DATE","RELEASE_DATE","START_DATE","CREATE_DATE","PUBLISH_DATE"});
		} 
	...
	public ModelAndView goEdit() throws Exception {
		...
		try {
			pd = getPageData();  
			pd = this.goversService.findById(pd); 
				List<PageData> userList = this.userService.listManager();
			    ...
			    mv.setViewName("system/Goverstment/Govers_edit"); 
		} catch (Exception e) { 
			e.printStackTrace();
		}  
		return mv;
