...
import com.fh.service.park.SettleParkService;
...
public class SettleParkController extends BaseController {  
	
	@Resource(name = "settleService")
	private SettleParkService settleService;   
	...
	public ModelAndView list(Page page) throws Exception {
		...
		mv.setViewName("park/settle_list");
		return mv;
	} 
	...
	public String queryList(Page page) throws Exception {
		...
		List<PageData> parks =this.settleService.listByParam(page);
		Object  total =this.settleService.findCount(page).get("counts");//查询记录的总行数  
		getObj.put("sEcho", initEcho);// 不知道这个值有什么用
		getObj.put("iTotalRecords", total);//实际的行数
		getObj.put("iTotalDisplayRecords", total);//显示的行数,这个要和上面写的一样
		
		for(PageData park:parks){ 
			ResultUtil.resetRes(park,new String[]{"AUDIT_DATE","MODIFY_DATE","CREATE_DATE"});
		} 
	 ...
	 public ModelAndView toEdit(){ 
		...
		try { 
			...
			if(pd.get("ID")!=null&&!"".equals(pd.get("ID"))){
				PageData data=this.settleService.findById(pd);
				mv.addObject("data", data);  
			}  
		} catch (Exception e) { 
			e.printStackTrace();
		} 
	    mv.setViewName("park/settle_edit"); 
	    return mv;
	 }   
	 
	 @RequestMapping(value={"/save"})
	 @SystemLog(methods="入驻园区管理",type="入驻编辑")
	 @ResponseBody
	 public String save(MultipartHttpServletRequest request){ 
		 ...
		 try {  
			 ...
			 String ENTERPRISE_NAME=new String(pd.getString("ENTERPRISE_NAME").getBytes("ISO-8859-1"),"UTF-8");
			 String YQ_NAME=new String(pd.getString("YQ_NAME").getBytes("ISO-8859-1"),"UTF-8");
			 String INTRODUCE=new String(pd.getString("ENTERPRISE_INTRODUCE").getBytes("ISO-8859-1"),"UTF-8");
			 String REQUIREMENT=new String(pd.getString("REQUIREMENT").getBytes("ISO-8859-1"),"UTF-8");
			 pd.put("ENTERPRISE_NAME",ENTERPRISE_NAME);
			 pd.put("YQ_NAME",YQ_NAME);
			 pd.put("ENTERPRISE_INTRODUCE",INTRODUCE); 
	         ...
	         if(ID==null||"".equals(ID)){//新增  
		    		...
		    		op=this.settleService.save(pd);  
	         }else{//修改  
	        	  pd.put("ID",ID);
	        	  pd.put("MODIFY_DATE",new Date());
		  		  pd.put("MODIFY_BY", getUser().getID());  
	    	 	  op=this.settleService.edit(pd);  
    	  	 } 
	         if(op){
	...
	@SystemLog(methods="入驻园区管理",type="删除")
	@ResponseBody
	public String delete() {
		...
		try {
			pd = getPageData(); 
			boolean resu=this.settleService.delete(pd);   
			if(resu){
				getObj.put("statusCode", "200"); 
			}else{
				getObj.put("statusCode", "400"); 
	...
	public ModelAndView toCheck(){
		...
		mv.setViewName("park/settle_check"); 
		return mv;
	}  
	
	@RequestMapping({ "/updateStatus" })
	@SystemLog(methods="入驻园区管理",type="审核")
	@ResponseBody
	public String updateStatus() {
		...
		try {
			...
			boolean resu=this.settleService.updateStatus(pd);
			if(!resu){
				obj.put("statusCode","400"); 
				return obj.toString();
			} 
			PageData user=this.settleService.queryBySid(pd); 
			if(user!=null){
				String content="";
				if("02".equals(pd.getString("STATUS"))){
					content="您提交的入驻园区申请审核未通过，快去消息中心查看原因吧！";
				}else{
					content="您提交的入驻园区申请已通过审核！";
				}
				PageData notif=new PageData(); 
			  	notif.put("ID", get32UUID()); 
				notif.put("CREATE_DATE", new Date());
	   	 		notif.put("TABLE_NAME","ZS_SETTLED_PARK");
