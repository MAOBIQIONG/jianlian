...
import com.fh.service.park.ParkImgService;
...
public class ParkImgController extends BaseController {  
	
	@Resource(name = "parkImgService")
	private ParkImgService imgService;   
	...
	public ModelAndView toLookImgs() throws Exception {
		...
		try {
			pd = getPageData();   
			List<PageData> imgs =imgService.queryByPid(pd);
			pd.put("ID",pd.getString("IN_ID"));
			PageData park=parkService.findById(pd);
			mv.addObject("IN_ID", pd.getString("IN_ID")); 
			mv.addObject("imgs", imgs); 
			mv.addObject("park", park); 
		} catch (Exception e) { 
			e.printStackTrace();
		}    
		mv.setViewName("park/park_img_list");  
		return mv;
	}
	...
	public ModelAndView toAdd() throws Exception {
	    ...
	    mv.setViewName("park/park_img_add");  
		return mv;
	}
	...
	public JSONObject uploadImgs(MultipartHttpServletRequest request)throws Exception{
		...
		pd.put("PRO_ID",IN_ID); 
		PageData data=imgService.queryMaxOrderby(pd);
	...
	public String delImg() throws Exception{
		...
		String[] ids= id.split(",");
		try {
			boolean ob=this.imgService.delete(ids);
			if(ob){
				obj.put("statusCode", "200");   
			}else{
				obj.put("statusCode", "400");   
