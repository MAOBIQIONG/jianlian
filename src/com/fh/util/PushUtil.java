...
public class PushUtil { 
    ...
    //对所有用户进行推送
    public String pushToAll(NotificationTemplate template) { 
...
//        template.setUrl("http://www.baidu.com");
  ...
  //对所有用户进行推送
    ...
    public String pushToMore(TransmissionTemplate tmtemplate,String[] cids) throws Exception {
    	...
    	//点击消息打开应用
    	//ListMessage message = ListMessageDemo();
    	//message.setData(nftemplate);
    ...
    public static TransmissionTemplate transmissionTemplateDemo(String title,String text) {
        ...
        //template.setTransmissionContent("");//payload、content内容
        // 设置定时展示时间
        ...
        payload.addCustomMsg("payloadJson", template.getTransmissionContent());
  ...
  //消息模板：透传消息
    public static TransmissionTemplate transmissionTemplateDemo(String titles,String texts,String jsonStr) {
    	String reg = "[^0-9a-zA-Z\u4e00-\u9fa5]+";
    	String title= titles.replaceAll(reg, "");
    	String text= texts.replaceAll(reg, "");
    	TransmissionTemplate template = new TransmissionTemplate();
        template.setAppId(appId);
        template.setAppkey(appKey);
        // 透传消息设置，1为强制启动应用，客户端接收到消息后就会立即启动应用；2为等待应用启动
