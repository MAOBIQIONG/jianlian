...
<![endif]-->
		<!--/meta 作为公共模版分离出去-->
		<title>修改行业分类信息</title>
	</head>

	<body>
	  <article class="page-container">
			<form class="form form-horizontal" id="form-cate-edit">
			    <input type="hidden" name="ID" id="ID" value="${pd.ID}" />
				<input type="hidden" name="PID" id="PId" value="${pd.PID}" /> 
				<div class="row cl">
					<label class="form-label col-xs-4 col-sm-2">名称：</label>
					<div class="formControls col-xs-8 col-sm-9"> 
						<input type="text" class="input-text" name="NAME" id="NAME" placeholder="这里输入名称" value="${pd.NAME }" data-rule-required="true"/>
					</div>
				</div>
				<!-- <div class="row cl">
					<label class="form-label col-xs-4 col-sm-2">描述：</label>
					<div class="formControls col-xs-8 col-sm-9"> 
					    <input type="text" class="input-text" name="DESCRIPTION" id="DESCRIPTION" placeholder="这里输入描述" value="${pd.DESCRIPTION}"/>
					</div>
				</div> -->
				<div class="row cl">
					<label class="form-label col-xs-4 col-sm-2">描述：</label>
					<div class="formControls col-xs-8 col-sm-9"> 
				       <textarea class="textarea"  placeholder=""  id="DESCRIPTION" name="DESCRIPTION">${pd.DESCRIPTION}</textarea> 
					</div>
				</div>
				<div class="row cl">
		...
		<script type="text/javascript">
		 $(function(){
			 var validate = $("#form-cate-edit").validate({ 
		 ...
		 function cate_save(){ 
				...
				$.ajax({
	                url : "<%=basePath%>category/save.do",
	                data :$('#form-cate-edit').serialize(),
	                type : 'post',
	                dataType : 'json',
	                async : false,
