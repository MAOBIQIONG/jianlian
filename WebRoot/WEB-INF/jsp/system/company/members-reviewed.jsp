...
<article class="page-container">
	...
	<input type="hidden" name="ID" id="ID" value="${data.ID }"/>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${data.EMAIL }" placeholder="" id="EMAIL" name="EMAIL" data-rule-required="true" data-rule-email="true">
			</div>
		</div> 
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>是否为会员：</label> 
			<div class="formControls col-xs-8 col-sm-9 skin-minimal" id="IS_VIP">
				<div class="radio-box">
					<input name="IS_VIP" type="radio" id="flag-1" value="1" <c:if test="${data.IS_VIP == '1' }">checked="checked"</c:if>>
					<label for="flag-1">是</label>
				</div>
				<div class="radio-box">
					<input type="radio" id="flag-2" name="IS_VIP" value="01" <c:if test="${data.IS_VIP == '01' }">checked="checked"</c:if>>
					<label for="flag-2">不是</label>
				</div> 
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${data.CARD_NO }" placeholder="" id="CARD_NO" name="CARD_NO" data-rule-required="true">
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9"> 
				<span class="select-box" style="width:150px;"> 
					<select class="select" name="MANAGER_ID" id="MANAGER_ID" data-placeholder="请选择项目经理" style="vertical-align:top;">
						<option value=""></option>
						<c:forEach items="${managerList}" var="manager">
							<option value="${manager.ID }" <c:if test="${manager.ID == data.MANAGER_ID}">selected</c:if>>${manager.NAME }</option>
						</c:forEach>
					</select>
				</span> 
			</div> 	
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9 skin-minimal" id="STATUS">
				<div class="radio-box">
					<input name="STATUS" type="radio" id="flag-1" value="1" <c:if test="${data.STATUS == '1' }">checked="checked"</c:if>>
					<label for="flag-1">审核通过</label>
				</div>
				<div class="radio-box">
					<input type="radio" id="flag-2" name="STATUS" value="2" <c:if test="${data.STATUS == '2' }">checked="checked"</c:if>>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9"> 
		       <textarea class="textarea"  placeholder=""  id="NOPASSREASON" name="NOPASSREASON">${data.NOPASSREASON}</textarea> 
			</div>
		</div> 
		<div class="row cl">
