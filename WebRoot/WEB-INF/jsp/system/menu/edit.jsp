	...
	<body>
	  <article class="page-container">
			<form class="form form-horizontal" id="form-menu-edit">
				...
				<input type="hidden" name="PARENT_ID" id="pId" value="${pd.PARENT_ID}" /> 
				<div class="row cl">
					<label class="form-label col-xs-4 col-sm-2">名称：</label>
					<div class="formControls col-xs-8 col-sm-9"> 
						<input type="text" class="input-text" name="MENU_NAME" id="menuName" placeholder="这里输入菜单名称" value="${pd.MENU_NAME }" data-rule-required="true"/>
					</div>
				</div>
				<div class="row cl">
					<label class="form-label col-xs-4 col-sm-2">链接：</label>
					<div class="formControls col-xs-8 col-sm-9"> 
					    <input type="text" class="input-text" name="MENU_URL" id="menuUrl" placeholder="这里输入链接地址" value="${pd.MENU_URL}" data-rule-required="true"/>
					</div>
				</div>
				<div class="row cl">
					<label class="form-label col-xs-4 col-sm-2">序号：</label>
					<div class="formControls col-xs-8 col-sm-9"> 
					    <input type="number" class="input-text"  style="width:200px;" name="MENU_ORDER" id="menuOrder" placeholder="这里输入序号" value="${pd.MENU_ORDER}" data-rule-required="true"/>
					</div>
				</div> 
				<div class="row cl"> 
					...
					<div class="formControls col-xs-8 col-sm-9 skin-minimal">
						<div class="radio-box">
							<input name="MENU_TYPE" id="form-field-radio1" onclick="setType('1');" <c:if test="${pd.MENU_TYPE == '1' }">checked="checked"</c:if> type="radio" value="1">
							<label for="type-1">系统菜单</label> 
						</div>
						<div class="radio-box"> 
							<input name="MENU_TYPE" id="form-field-radio2" onclick="setType('2');" <c:if test="${pd.MENU_TYPE != '1' }">checked="checked"</c:if> type="radio" value="0">
						    <label for="type-2">业务菜单</label>
						</div>
					</div> 
				</div> 
				<div class="row cl">
		...
		<script type="text/javascript">
		 $(function(){
			 var validate = $("#form-menu-edit").validate({ 
