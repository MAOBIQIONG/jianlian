...
<![endif]-->
	...
	<body class="pos-r"> 
		<article class="page-container">
			<form  class="form form-horizontal"  id="form-model-edit">
			    <input type="hidden" name="ID" id="id" value="${pd.ID }"/> 
			    <input type="hidden" name="FILE_NAME" id="FILE_NAME"  value="${pd.FILE_NAME }"/>  
				<div class="row cl">
					<label class="form-label col-xs-4 col-sm-2">合同模板名称：</label>
					<div class="formControls col-xs-8 col-sm-9"> 
					    <input type="text" class="input-text" name="MODEL_NAME" id="MODEL_NAME"  value="${pd.MODEL_NAME }" placeholder="这里输入名称" data-rule-required="true"/>
					</div>
				</div>
				<div class="row cl">
					<label class="form-label col-xs-4 col-sm-2">合同模板类型：</label>
					<div class="formControls col-xs-8 col-sm-9">
					    <input type="text" class="input-text" name="MODEL_TYPE" id="MODEL_TYPE" value="${pd.MODEL_TYPE }" placeholder="输入类型" data-rule-required="true"/> 
					</div>
				</div>
				<div class="row cl">
					...
					<div class="formControls col-xs-8 col-sm-9"> 
						<span class="btn-upload form-group"> 
							<input type="file" id="model" name="modelFile" style="width:150px;" data-rule-required="true"/>
						</span> 
					</div>
				</div>
				<div class="row cl">
       ...
       <!--_footer 作为公共模版分离出去-->
		...
		<script type="text/javascript">
		 $(function(){
			 var validate = $("#form-model-edit").validate({ 
		 ...
		 function save(){ 
				...
				var url="<%=basePath%>contractModel/uploadFile.do";
				var URL=url+"?MODEL_NAME="+model_name+"&MODEL_TYPE="+model_type+"&FILE_NAME="+FILE_NAME+"&ID="+Id;
				$.ajaxFileUpload({  
					  url :URL,
					  type:'post',
		              secureuri:false,  
		              fileElementId:'model',//file标签的id  
