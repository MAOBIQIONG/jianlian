...
<![endif]-->
	...
	<body>
		<article class="page-container">
			<form class="form form-horizontal" id="form-pay-edit"> 
				<div class="row cl">
					<input type="hidden" name="ID" id="ID" value="${pd.ID }"/>
					<input type="hidden" name="STATUS" id="STATUS" value="01"/> 
					<input type="hidden" name="CHANNEL" id="CHANNEL" value="alipay"/>     
					    ...
					    <span class="select-box" style="width:150px;"> 
							<select class="select" name="USER_ID" id="USER_ID"> 
								<c:forEach items="${userList}" var="user">
									<option value="${user.ID }" <c:if test="${user.ID ==pd.USER_ID}">selected</c:if>>${user.REAL_NAME }</option>
								</c:forEach>
							</select>
						</span> 
					</div>
				...
				<div class="row cl">
					...
					<div class="formControls col-xs-8 col-sm-9">
						<input type="number" style="width:150px;" class="input-text" value="${pd.PRICE}" placeholder="" id="PRICE" name="PRICE" data-rule-required="true">
					</div>
				</div>
				<div class="row cl">
					<label class="form-label col-xs-4 col-sm-2">事件：</label>
					<div class="formControls col-xs-8 col-sm-9">
						<input type="text" class="input-text" value="${pd.EVENT}" placeholder="" id="EVENT" name="EVENT" data-rule-required="true">
					</div>
				</div> 
				<div class="row cl">
					<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
						<input onclick="pay_save" class="btn btn-primary radius" type="button" value="&nbsp;&nbsp;提交&nbsp;&nbsp;"> 
					</div>
				</div>
			</form>
		</article>
