...
<article class="page-container" style="width: 700px;">
	...
	<input type="hidden" name="ID" id="ID" value="${mediaData.ID }"/>
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input onblur="showdateInput()" type="text" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd'})" class="input-text Wdate" value="${mediaData.PUBLISH_DATE }" placeholder="" id="PUBLISH_DATE" name="PUBLISH_DATE" data-rule-required="true">
			</div>
		</div>
		 <div class="row ">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9 skin-minimal" id="media-mors">
				<div class="radio-box">
					<input name="MORS" type="radio"  id="jzxw" class="MORS"  value="2"  <c:if test="${mediaData.MORS == '2' }">checked="checked"</c:if>>
					<label for="flag-1">建筑新闻</label>
				</div>
				<div class="radio-box">
					<input type="radio" class="MORS" id="jltt" name="MORS" value="1"  <c:if test="${mediaData.MORS == '1' }">checked="checked"</c:if>>
					<label for="flag-2">建联头条</label>
				</div> 
			</div>
		</div> 
		<div class="row cl">
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9 skin-minimal" id="is-top1">
				<div class="radio-box">
					<input name="IS_TOP1" type="radio" class="IS_TOP" value="1"  <c:if test="${mediaData.IS_TOP == '1' }">checked="checked"</c:if>>
					<label for="flag-1">置顶</label>
				</div>
				<div class="radio-box">
					<input type="radio" class="IS_TOP" name="IS_TOP1" value="2"  <c:if test="${mediaData.IS_TOP == '2' }">checked="checked"</c:if>>
		...
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>媒体简介：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${mediaData.SUMMARY }" placeholder="" id="SUMMARY" name="SUMMARY" data-rule-required="true">
			</div>
		</div> 
		<div class="row cl">
	...
	<div style="background:url(<%=basePath%>images/phone.png) no-repeat;background-size:100%;width: 350px;height: 680px;position:fixed;">
		<div style="margin-top: 163px;height: 366px;margin-left: 22px;margin-right: 23px;">
			<div style="width: 90%;margin-left: 5%;margin-right: 5%;height: 365px;overflow-Y:auto;">
				<span id="heto" style="font-size: 16px;color: black;">请输入标题...</span>
				<div style="display:-webkit-box;">
					<span style="font-size: 14px;">上海建联官方</span>
					<span id="showdates" style="font-size: 10px;">
				...
				<hr style="color: black;border: none;height: 1px;background-color: #f2f2f2;margin-bottom: 0px;"/>
						<img id="shipinshow" style="width: 100%;display:none" src="<%=basePath%>images/videoshow.png">
				<div id="hecon" style="height: 315px;overflow: auto;">
				请输入内容...
				</div>
			</div>
		</div>
...
$(function(){
	var value=$("input[name='MORS']:checked").val();
	if(value=='2'){ 
		 $("#column-dh").hide(); 
	}else{
		$("#column-dh").show();
...
function showdateInput(){
    var biaoti = document.getElementById( "PUBLISH_DATE" );   // 取得输入框对象
    var zxc = document.getElementById( "showdates" );  // 取得DIV对象
    zxc.innerText = biaoti.value;   
}  
...
function checkVideo(){ 
	var file = $("#VIDEOS").val();
	/*$("#videoPlay").attr("src",file.replace(/\\/g,"/"));*/
		if(file!=null){
			$("#shipinshow").css("display","block");
		}else{
			$("#shipinshow").css("display","none");
		}
	 ...
	 if(aa[aa.length-1]=='mp4'){//判断视频格式
       ...
       return false;
		...
		function companyAdd(){ 
			...
			var MORS = $('#media-mors input[name="MORS"]:checked ').val(); 
			if( !MORS ){
				alert("请选择类别!");
				return;
			}
			...
			var IS_TOP = $('#is-top1 input[name="IS_TOP1"]:checked ').val(); 
			if( !IS_TOP ){
				alert("请确认是否置顶!");
				return;
			}
			...
			var url="<%=basePath%>Media/addmediaImgsAndVedios.do?ID="+ID; 
			 var map = {
			  ...
			  //var param="&TITLE="+TITLE+"&CONETENT="+content+"&PUBLISH_DATE="+PUBLISH_DATE+"&TYPE_ID="+TYPE_ID+"&LINK_URL="+LINK_URL+"&RESOURCE="+RESOURCE+"&COLUMN_ID="+COLUMN_ID+"&IS_RECOMMEND="+IS_RECOMMEND+"&IS_TOP="+IS_TOP+"&SUMMARY="+SUMMARY;
		      $.ajaxFileUpload({  
				  url : url,
				  type:'post',
	             secureuri:false,  
