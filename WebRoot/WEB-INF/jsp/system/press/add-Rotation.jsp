...
<![endif]-->
	...
	<body class="pos-r"> 
		<article class="page-container">
			<form class="form form-horizontal" id="form-rotation-edit" > 
			<input type="hidden" name="ID" id="ID" value="${RotationData.ID }"/> 
			<div class="row cl">
				<label class="form-label col-xs-4 col-sm-2">图片名称：</label>
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text" value="${RotationData.TITLE }" placeholder="" id="TITLE" name="TITLE">
				</div>
			</div> 
			<div class="row cl">
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9 skin-minimal" id="rolation">
					<div class="radio-box">
						<input name="IS_SHOW" type="radio" class="IS_SHOW" value="1"  <c:if test="${RotationData.IS_SHOW == '1' }">checked="checked"</c:if>>
						<label for="flag-1">是</label>
					</div>
					<div class="radio-box">
						<input type="radio" class="IS_SHOW"  name="IS_SHOW" value="2"  <c:if test="${RotationData.IS_SHOW == '2' }">checked="checked"</c:if>>
			...
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
					<input type="text" class="input-text" value="${RotationData.LINK_URL }" placeholder="" id="LINK_URL" name="LINK_URL">
				</div>
			</div> 
			<div class="row cl">
				...
				<div class="formControls col-xs-8 col-sm-9">
			        <span class="select-box" style="width:150px;"> 
						<select class="select" name="COLUMN_ID" id="COLUMN_ID"> 
							<c:forEach items="${lianlist}" var="press">
								<option value="${press.ID }" <c:if test="${press.ID == RotationData.COLUMN_ID }">selected</c:if>>${press.TITLE_CATEGORY_NAME }</option>
							</c:forEach>
						</select>
					</span> 
				</div>
	...
	$(function(){
		 var validate = $("#form-rotation-edit").validate({ 
	 ...
	 function check(){ 
		 ...
		 function companyAdd(){
			...
			if(img==null||img==""||img==undefined){
				...
				var url="<%=basePath%>Rotation/addrotation.do";
			   	var URL=url+"?ID="+ID+"&TITLE="+TITLE+"&IS_SHOW="+IS_SHOW+"&LINK_URL="+LINK_URL+"&COLUMN_ID="+COLUMN_ID; 
				 $.ajaxFileUpload({  
					  url :URL,
					  type:'post',
		              secureuri:false,  
