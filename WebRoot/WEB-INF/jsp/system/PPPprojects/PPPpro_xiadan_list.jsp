		...
		<div> 
			...
			<div class="page-container">
				...
				<div class="cl pd-5 bg-1 bk-gray mt-20">
					<span class="l">
					 <%--  <c:if test="${QX.add == 1 }">  --%>
					    <a class="btn btn-primary radius" data-title="项目发布" onclick="project_edit('项目发布','<%=basePath%>PPPproject/toAdd.do','','550')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 项目发布</a>	
					 <%--  </c:if>  
					  <c:if test="${QX.edit == 1 }">   --%>
					    <a href="javascript:void(0)" onclick="project_check('审核','<%=basePath%>PPPproject/toCheck.do','','550')" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i>审核</a> 
					<%--   </c:if>  
					  <c:if test="${QX.edit == 1 }">   --%>
					    <a href="javascript:void(0)" onclick="project_check('分配','<%=basePath%>PPPproject/toDis.do','','550')" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i>分配</a> 
					  <%-- </c:if>    --%> 
					   <a href="javascript:void(0)" onclick="project_check('查看','<%=basePath%>PPPproject/toLook.do','','550')" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe600;</i>查看</a> 
					</span>  
				</div> 
				<div class="mt-20">
				...
				{  
                 ...
                 {  
                     ...
                     "mRender":function(a,b,c,d){//a表示statCleanRevampId对应的值，c表示当前记录行对象  
                         return '<a _href=\"<%=basePath%>pppxmtp/toLookImgs.do?ID='+c.ID+'\" data-title=\"查看ppp项目图片\" class=\"btn btn-link\" onclick=\"Hui_admin_tab(this)\" href=\"javascript:;\"  >查看图片</a>';
                      }  
	              }, 
                 {
                     "sClass":"center",  
                     "aTargets":[14],  
                     "mRender":function(a,b,c,d){//a表示statCleanRevampId对应的值，c表示当前记录行对象  
                     	return '<a data-title=\"编辑\" onclick=\"project_edit(\'编辑\',\'<%=basePath%>PPPproject/goEdit.do?ID='+c.ID+'\',\'\',\'300\')\" href=\"javascript:;\" class=\"btn btn-secondary radius\" style=\"text-decoration:none\"><i class=\"Hui-iconfont\">&#xe6df;</i> 编辑</a>&nbsp;&nbsp;<a data-title=\"删除\" href=\"javascript:;\" onclick=\"project_del(\'<%=basePath%>PPPproject/delete.do?ID='+c.ID+'\')\" class=\"btn btn-secondary radius\" style=\"text-decoration:none\"><i class=\"Hui-iconfont\">&#xe6e2;</i> 删除</a>'; 
                    }  
                 }
				], 
