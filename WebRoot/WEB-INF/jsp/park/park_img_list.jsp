...
<body class="pos-r">  
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>产业园区管理 <span class="c-gray en">&gt;</span>图片展示
<a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px"  href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="page-container"> 
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
	    <span class="l">
	        <a href="javascript:;" onclick="img_del('<%=basePath%>parkImg/delImg.do?IN_ID=${IN_ID}')" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a>  
		    <a id="img-add" class="btn btn-primary radius" data-title="添加图片" onclick="img_edit('添加图片','<%=basePath%>parkImg/toAdd.do?IN_ID=${IN_ID}','','550')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i>添加图片</a>
		</span> 
	</div>
				...
				<c:choose>
					<c:when test="${not empty imgs}"> 
						<c:forEach items="${imgs}" var="img"> 
						   <c:if test="${park.USER_ID !=null && park.USER_ID !=''}">  
							...
							<c:if test="${park.CREATE_BY !=null && park.CREATE_BY !=''}">  
						        <c:if test="${img.IMG_PATH !=null && img.IMG_PATH !=''}"> 
						        	<c:choose>
										 <c:when test="${fn:contains(img.IMG_PATH,'http://')}"> 
											<li class="item">
